...
def split_composite_rule(composite_rule: str) -> list[str]:
    """Split composite rule into normalized values.

    Rules are applied retrosynthetically in their encoded order; failures are raised as
    route unwrapping errors so callers can report the skipped target.
    """
...
def first_retro_reaction(reactor: Any, molecule: Any) -> Any | None:
    """Return the first retrosynthetic reaction generated by a reactor.
...
def select_retro_reaction(
    ...
    """Select retro reaction for the next processing step.
...
def molecule_node(molecule: Any, *, in_stock: bool = False) -> dict[str, Any]:
    """Return molecule node from a molecule or molecule node.
...
def mark_leaf_molecules_in_stock(node: dict[str, Any]) -> None:
    """Mark leaf molecules in stock in an unwrapped route tree.
...
def unwrap_rule_sequence(
    ...
    *,
    ...
    """Unwrap rule sequence into a retrosynthetic route.
    ...
    def collect_leaf_molecules(node: dict[str, Any]) -> None:
        """Collect leaf molecules from route-analysis input data.

        Rules are applied retrosynthetically in their encoded order; failures are raised
        as route unwrapping errors so callers can report the skipped target.
        """
        if node.get("type") == "mol" and not node.get("children"):
